...
std::shared_ptr<ChSuspension> ReadSuspensionJSON(const std::string& filename) {
    ...
    } else if (subtype.compare("TwistBeam") == 0) {
        suspension = chrono_types::make_shared<TwistBeam>(d);
    } else {
...
std::shared_ptr<ChTire> ReadTireJSON(const std::string& filename) {
    ...
    if (subtype.compare("RigidTire") == 0) {
        ...
        tire = std::make_shared<Pac89Tire>(d); 
    ...
    } else if (subtype.compare("MFTyre62") == 0) {
        tire = chrono_types::make_shared<MFTyre62>(d);
    } else {
