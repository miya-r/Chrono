...
class CH_VEHICLE_API ChChassis : public ChPart {
  public:
    ...
    virtual void Synchronize(double time, ChVector<> &act_fforce, ChVector<> & act_ftorque);
            
    /// Get MeshFileName & MeshName
    virtual const std::string& GetMeshName() const = 0;
    virtual const std::string& GetMeshFilename() const = 0;
