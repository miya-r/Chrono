...
void WheeledVehicle::Create(const std::string& filename) {
    ...
    // -------------------------------------------
    GetLog() << "start create WheeledVehicle\n";
    ...
    // ----------------------------
    GetLog() << "Validations of JSON file\t";
    ...
    // -------------------------------------------
    GetLog() << "create Chassis\t";
    {
        std::string file_name = d["Chassis"]["Input File"].GetString();
        m_chassis = ReadChassisJSON(vehicle::GetinputDirDataFile(file_name));
...
GetLog() << "complete\n";
    ...
    // --------------------
GetLog() << "create driveline\t";
    {
        std::string file_name = d["Driveline"]["Input File"].GetString();
        m_driveline = ReadDrivelineJSON(vehicle::GetinputDirDataFile(file_name));
...
GetLog() << "complete\n";
    ...
    // ---------------------------------------------------
GetLog() << "create suspension\t";
    for (int i = 0; i < m_num_axles; i++) {
        // Suspension
        std::string file_name = d["Axles"][i]["Suspension Input File"].GetString();
        m_suspensions[i] = ReadSuspensionJSON(vehicle::GetinputDirDataFile(file_name));
        ...
        if (d["Axles"][i].HasMember("Antirollbar Input File")) {
            ...
            m_antirollbars.push_back(ReadAntirollbarJSON(vehicle::GetinputDirDataFile(file_name)));
        ...
        m_wheels[2 * i + VehicleSide::LEFT] = ReadWheelJSON(vehicle::GetinputDirDataFile(file_name));
        ...
        m_wheels[2 * i + VehicleSide::RIGHT] = ReadWheelJSON(vehicle::GetinputDirDataFile(file_name));
        ...
        m_brakes[2 * i + VehicleSide::LEFT] = ReadBrakeJSON(vehicle::GetinputDirDataFile(file_name));
        ...
        m_brakes[2 * i + VehicleSide::RIGHT] = ReadBrakeJSON(vehicle::GetinputDirDataFile(file_name));
    ...
    if (d.HasMember("Traction joint")) {
        m_traction_joint_pos = ReadVectorJSON(d["Traction joint"]);
    }
