...
#ifndef CH_TWISTBEAM_IRS_H
#define CH_TWISTBEAM_IRS_H
...
/// suspension and will be mirrored (reflecting the y coordinates) to construct
/// the right side.
class CH_VEHICLE_API ChTwistBeam : public ChSuspension {
  public:
    ChTwistBeam(const std::string& name ); ///< [in] name of the subsystem
                
    virtual ~ChTwistBeam() {}

    /// Get the name of the vehicle subsystem template.
    virtual std::string GetTemplateName() const override { return "TwistBeam"; }
  ...
  protected:
    ...
    enum PointId {
        ...
        TB_A,      ///< twist beam, connection to arm
        TB_CM,     ///< twist beam, com
    ...
    virtual double getBeamMass() const = 0;

    /// Return the rotational spring coefficient.
    virtual double getBeamSpringCoefficient() const = 0;
    /// Return the rotational damping coefficient.
    virtual double getBeamDampingCoefficient() const = 0;
    ...
    virtual const ChVector<>& getBeamInertia() const = 0;


    /// Return the inertia of the axle shaft.
    virtual double getAxleInertia() const = 0;

    virtual const ChVector<> getBeamCOM() const = 0;

    /// Return the radius of the trailing arm body (visualization only).
    virtual double getArmRadius() const = 0;

    virtual double getBeamRadius() const = 0;
    ...
    std::shared_ptr<ChBody> m_beam[2];    ///< handles to the Twist Beam

    std::shared_ptr<ChLinkLockSpherical> m_sphericalArm[2];    ///< chassis-arm revolute joints (left/right)
    std::shared_ptr<ChLinkLockLock> m_lockBeam[2];    ///< chassis-arm revolute joints (left/right)
    std::shared_ptr<ChLinkLockRevolute> m_revoluteBeam;     ///< handle to central revolute joint
