...
////         auto my_force = std::unique_ptr<ChLinkForce>();
...
ChLinkForce& ChLinkLock::GetForce_D() {
    if (!force_D)
        force_D = std::unique_ptr<ChLinkForce>();
    return *force_D;
}
ChLinkForce& ChLinkLock::GetForce_R() {
    if (!force_R)
        force_R = std::unique_ptr<ChLinkForce>();
    return *force_R;
}
ChLinkForce& ChLinkLock::GetForce_X() {
    if (!force_X)
        force_X = std::unique_ptr<ChLinkForce>();
    return *force_X;
}
ChLinkForce& ChLinkLock::GetForce_Y() {
    if (!force_Y)
        force_Y = std::unique_ptr<ChLinkForce>();
    return *force_Y;
}
ChLinkForce& ChLinkLock::GetForce_Z() {
    if (!force_Z)
        force_Z = std::unique_ptr<ChLinkForce>();
    return *force_Z;
}
ChLinkForce& ChLinkLock::GetForce_Rx() {
    if (!force_Rx)
        force_Rx = std::unique_ptr<ChLinkForce>();
    return *force_Rx;
}
ChLinkForce& ChLinkLock::GetForce_Ry() {
    if (!force_Ry)
        force_Ry = std::unique_ptr<ChLinkForce>();
    return *force_Ry;
}
ChLinkForce& ChLinkLock::GetForce_Rz() {
    if (!force_Rz)
        force_Rz = std::unique_ptr<ChLinkForce>();
    return *force_Rz;
}
...
//// If ever needed, these functions can be re-enabled. In that case,
//// a typical user call would be:
////         auto my_limit = std::unique_ptr<ChLinkLimit>();
...
ChLinkLimit& ChLinkLock::GetLimit_X() {
    if (!limit_X)
        limit_X = std::unique_ptr<ChLinkLimit>();
    return *limit_X;
}
ChLinkLimit& ChLinkLock::GetLimit_Y() {
    if (!limit_Y)
        limit_Y = std::unique_ptr<ChLinkLimit>();
    return *limit_Y;
}
ChLinkLimit& ChLinkLock::GetLimit_Z() {
    if (!limit_Z)
        limit_Z = std::unique_ptr<ChLinkLimit>();
    return *limit_Z;
}
ChLinkLimit& ChLinkLock::GetLimit_Rx() {
    if (!limit_Rx)
        limit_Rx = std::unique_ptr<ChLinkLimit>();
    return *limit_Rx;
}
ChLinkLimit& ChLinkLock::GetLimit_Ry() {
    if (!limit_Ry)
        limit_Ry = std::unique_ptr<ChLinkLimit>();
    return *limit_Ry;
}
ChLinkLimit& ChLinkLock::GetLimit_Rz() {
    if (!limit_Rz)
        limit_Rz = std::unique_ptr<ChLinkLimit>();
    return *limit_Rz;
}
ChLinkLimit& ChLinkLock::GetLimit_Rp() {
    if (!limit_Rp)
        limit_Rp = std::unique_ptr<ChLinkLimit>();
    return *limit_Rp;
}
ChLinkLimit& ChLinkLock::GetLimit_D() {
    if (!limit_D)
        limit_D = std::unique_ptr<ChLinkLimit>();
    return *limit_D;
}
